...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	int xPoint = workArea.left + (workAreaWidth - WIN_WIDTH) / 2;
	int yPoint = workArea.top + (workAreaHeight - WIN_HEIGHT) / 2;

	// create the window
	// CreateWindowEX is also there to use when we want give extra styles
	SR_hwnd = CreateWindow(SR_lpszAppName,
						   TEXT("RTR7-015-Sagar-Raut-MyProjects-01-OpenGL-01-FFP-01-Windows-01-Windowing-Centering-SystemParameterInfo"),
						   WS_OVERLAPPEDWINDOW,
						   xPoint,	   // x
						   yPoint,	   // y
						   WIN_WIDTH,  // width

...
int main(void)
{
    ...
    {
        iArray[SR_i] = (int *)malloc((NUM_COLUMNS - SR_i) * sizeof(int));
        if (iArray[SR_i] == NULL)
        {
    ...
    {
        for (SR_j = 0; SR_j < (NUM_COLUMNS - 1); SR_j++)
        {
    ...
    for (SR_i = 0; SR_i < 5; SR_i++)
    {
        for (SR_j = 0; SR_j < (NUM_COLUMNS - SR_i); SR_j++)
        {
            printf("iArray[%d][%d] = %d \t ar address %p\n", SR_i, SR_j, iArray[SR_i][SR_j], &iArray[SR_i][SR_j]);
        }
        printf("\n\n");
    }
